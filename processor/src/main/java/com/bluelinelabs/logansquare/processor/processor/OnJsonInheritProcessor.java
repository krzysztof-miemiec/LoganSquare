...
public class OnJsonInheritProcessor extends MethodProcessor {

    public OnJsonInheritProcessor(ProcessingEnvironment processingEnv) {
        super(processingEnv);
    }

    @Override
    public Class getAnnotation() {
        return OnJsonInherit.class;
    }
    ...
    public void findAndParseObjects(RoundEnvironment env, Map<String, JsonObjectHolder> jsonObjectMap, Elements elements, Types types) {
        for (Element element : env.getElementsAnnotatedWith(OnJsonInherit.class)) {
            try {
    ...
    private void processOnPreJsonSerializeMethodAnnotation(Element element, Map<String, JsonObjectHolder> jsonObjectMap, Elements elements) throws Exception {
        if (!isCallbackMethodAnnotationValid(element, OnJsonInherit.class.getSimpleName())) {
            return;
        }
        ...
        objectHolder.onInheritCallback = executableElement;
        objectHolder.inheritsFromParent = true;
    }
